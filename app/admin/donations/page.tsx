  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center p-4">
        <Card className="premium-card p-6 w-full max-w-sm">
          <div className="flex items-center justify-center mb-6">
            <Lock className="w-8 h-8 text-[hsl(var(--accent))] mr-2" />
            <h1 className="text-xl font-bold text-[hsl(var(--accent))]">Admin Login</h1>
          </div>
          ...
          <form onSubmit={handleLogin}>
            <Input
              ...
              className="mb-4 bg-[hsl(var(--surface-2))] border-[hsl(var(--border-subtle))] text-white placeholder:text-[hsl(var(--text-muted))]"
              autoFocus
            />
            ...
            <Button
              ...
              className="w-full premium-button-accent"
            >
              {loginLoading ? (
    ...
    <div className="min-h-screen bg-black p-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-[hsl(var(--accent))]">
            Cashu Donations
          </h1>
        ...
        {stats && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <Card className="premium-card p-4">
              <div className="text-[hsl(var(--text-muted))] text-xs mb-1">Unredeemed</div>
              <div className="text-green-400 text-xl font-bold font-mono">
                {stats.unredeemedAmount.toLocaleString()} sats
              </div>
            </Card>
            <Card className="premium-card p-4">
              <div className="text-[hsl(var(--text-muted))] text-xs mb-1">Redeemed</div>
              <div className="text-[hsl(var(--text-muted))] text-xl font-bold font-mono">
                {stats.redeemedAmount.toLocaleString()} sats
              </div>
            </Card>
            <Card className="premium-card p-4">
              <div className="text-[hsl(var(--text-muted))] text-xs mb-1">Tokens</div>
              <div className="text-white text-xl font-bold font-mono">
        ...
        <div className="space-y-2">
          {loading && tokens.length === 0 ? (
            ...
            <Card className="premium-card p-8 text-center">
              <div className="text-[hsl(var(--text-muted))]">No donations yet</div>
            </Card>
          ) : (
            tokens.map((token) => (
              <Card
                ...
                } ${selectedTokens.has(token.id) ? "ring-2 ring-[hsl(var(--accent))]" : ""}`}
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span
                        className={`text-lg font-bold font-mono ${token.redeemed ? "text-[hsl(var(--text-muted))]" : "text-white"}`}
                      >
                        {token.amount > 0
                    ...
                    <div className="text-xs text-[hsl(var(--text-muted))] mb-2">
                      {formatDate(token.timestamp)}
                    </div>

                    {/* Note */}
                    {token.note && (
                      <div className="text-sm text-[hsl(var(--accent))] bg-[hsl(var(--accent)/0.1)] border border-[hsl(var(--accent)/0.2)] rounded px-2 py-1 mb-2">
                        <span className="text-[hsl(var(--text-muted))] text-xs mr-1">Note:</span>
                        {token.note}
                      </div>
                    )}
                    ...
                    <div className="flex items-center gap-2">
                      <code className="text-xs text-[hsl(var(--text-muted))] bg-[hsl(var(--surface-2))] px-2 py-1 rounded flex-1 overflow-hidden font-mono">
                      ...
                      <Button
                        ...
                        variant="ghost"
                        size="sm"
                        className="text-[hsl(var(--text-muted))] hover:text-white p-1 h-auto"
                      >
                        {showTokens.has(token.id) ? (
        ...
        {/* Instructions */}
        <Card className="premium-card p-4 mt-6">
          <h3 className="text-[hsl(var(--accent))] font-medium mb-2">How to redeem</h3>
          <ol className="text-[hsl(var(--text-muted))] text-sm space-y-1 list-decimal list-inside">
