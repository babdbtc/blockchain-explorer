...
export function SearchModal({ isOpen, onClose, query }: SearchModalProps) {
  ...
  const StatCard = ({ label, value, icon: Icon, subValue }: any) => (
    <div className="bg-[hsl(var(--surface-2))] p-3 rounded-lg border border-[hsl(var(--border-subtle))] flex items-center space-x-3">
      <div className="p-2 bg-[hsl(var(--accent)/0.1)] rounded-full">
              ...
              <div key={i} className="bg-[hsl(var(--surface-2))] p-3 rounded border border-[hsl(var(--border-subtle))] text-sm group hover:border-[hsl(var(--accent)/0.3)] transition-colors">
                ...
                {vin.prevout?.scriptpubkey_address ? (
                  <Link
                    ...
                    className="text-xs text-gray-300 font-mono break-all opacity-70 group-hover:opacity-100 transition-opacity hover:text-[hsl(var(--accent))] hover:underline block"
                  >
                    {vin.prevout.scriptpubkey_address}
                  </Link>
                ) : (
                  <div className="text-xs text-[hsl(var(--text-muted))] font-mono italic opacity-50">
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1 min-w-0 mr-4">
                    <div className="flex items-center space-x-2 mb-1">
                      <span className="text-xs text-[hsl(var(--text-muted))]">UTXO #{i + 1}</span>
      ...
      <DialogContent className="premium-modal text-white max-w-4xl max-h-[85vh] overflow-y-auto">
        ...
        <div className="mt-4 space-y-6">
          <div className="relative group">
            <div className="p-3 bg-[hsl(var(--surface-2))] rounded-lg border border-[hsl(var(--border-subtle))] font-mono text-xs text-gray-300 break-all pr-10">
              {query}
            </div>
            <Button
              size="icon"
              variant="ghost"
              className="absolute right-1 top-1 h-7 w-7 text-[hsl(var(--text-muted))] hover:text-white hover:bg-[hsl(var(--surface-3))]"
          ...
          {loading && (
            <div className="flex flex-col items-center justify-center py-12 space-y-4">
              <div className="relative">
                <div className="absolute inset-0 bg-[hsl(var(--accent)/0.2)] blur-xl rounded-full animate-pulse" />
                <Loader2 className="w-10 h-10 animate-spin text-[hsl(var(--accent))] relative z-10" />
              </div>
              <span className="text-sm text-[hsl(var(--text-muted))] animate-pulse">Fetching blockchain data...</span>
            </div>
          )}
